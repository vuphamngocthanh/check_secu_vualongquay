 ...
 // script để thực hiện quá trình upload
	...
	*/
	function showFormUpload() {
		...
		<form action="'.htmlentities($_SERVER['PHP_SELF']).'?dem='.$_GET['ids'].'" method="post" enctype="multipart/form-data" onsubmit="wait()">
	...
	*/
	function getLink($i) {
		...
		$return .= "<tr><td colspan=\"2\"><a href=\"javascript:window.opener.document.adminForm.userfile$ids.value='$link_bs';window.close();window.history.go(1);\">close</a></td> </tr></table>";
		return $return;
